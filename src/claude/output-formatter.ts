...
export function createToolApprovalEmbed(
  ...
  const embed = new EmbedBuilder()
    .setTitle(L(`🔧 Tool Use: ${toolName}`, `🔧 工具使用：${toolName}`))
    .setColor(0xffa500)
    .setTimestamp();
  ...
  if (toolName === "Edit" || toolName === "Write") {
    ...
    embed.addFields({ name: L("File", "文件"), value: `\`${filePath}\``, inline: false });

    if (input.old_string && input.new_string) {
      const diff = `\`\`\`diff\n- ${String(input.old_string).slice(0, 500)}\n+ ${String(input.new_string).slice(0, 500)}\n\`\`\``;
      embed.addFields({ name: L("Changes", "更改内容"), value: diff, inline: false });
    } else if (input.content) {
      const preview = String(input.content).slice(0, 500);
      embed.addFields({
        name: L("Content Preview", "内容预览"),
        value: `\`\`\`\n${preview}\n\`\`\``,
        inline: false,
      });
    }
    ...
    embed.addFields(
      { name: L("Command", "命令"), value: `\`\`\`bash\n${command}\n\`\`\``, inline: false },
    );
    if (description) {
      embed.addFields({ name: L("Description", "描述"), value: description, inline: false });
    }
  } else {
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
    ...
    new ButtonBuilder()
      ...
      .setLabel(L("Auto-approve All", "全部自动批准"))
      .setStyle(ButtonStyle.Secondary)
      .setEmoji("⚡"),
  );
...
export function createAskUserQuestionEmbed(
  ...
  if (questionData.multiSelect) {
    ...
    const selectMenu = new StringSelectMenuBuilder()
      ...
      .setPlaceholder(L("Select options...", "请选择一个选项..."))
      .setMinValues(1)
...
export function createResultEmbed(
  ...
  const footer = showCost
    ? `${L("Cost (est.)", "费用（估计）")} : $${costUsd.toFixed(4)}  |  ${L("Duration", "耗时")} : ${duration}`
    : `${L("Duration", "耗时")} : ${duration}`;

  const embed = new EmbedBuilder()
    .setTitle(L("✅ Task Complete", "✅ 任务完成"))
